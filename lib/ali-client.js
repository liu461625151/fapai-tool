...
function formatItem(item) {
  ...
  return {
    ...
    viewCount: item.pv || 0,
    picUrl: item.picURL ? `https://img.alicdn.com/imgextra/${item.picURL}` : '',
    picUrls: (em.headerPicUrls ? em.headerPicUrls.split(',').map(p => `https://img.alicdn.com/imgextra/${p.trim()}`) : (item.picURL ? [`https://img.alicdn.com/imgextra/${item.picURL}`] : [])),
    coordinate: item.coordinate || [],
    tags: (item.tags || []).map(t => t.alias).filter(Boolean),
    discountTag: discountTag,
    source: item.auctionSource || '',
 ...
 * 搜索司法拍卖房产
 */
async function searchJudicial({ page = 1, province = '', city = '', district = '', status = '0,1', category = '' }) {
